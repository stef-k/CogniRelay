...
- The intended usage convention is that collaborator agents interact through the coordination surfaces (handoffs, shared coordination artifacts, messaging) rather than directly reading the owner's continuity capsules. Until sub-directory namespace granularity ships ([#149](https://github.com/stef-k/CogniRelay/issues/149)), the `collaboration_peer` template grants `memory` namespace read access, which technically includes continuity data. Operators who want stricter isolation before #149 ships should issue tokens with more restricted read namespaces.
