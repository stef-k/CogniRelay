...
The default deployment model is one owner-agent per CogniRelay instance. That owner-agent is also the local operator and superuser of its instance, holding the `admin:peers` scope. Continuity capsules are the owner-agent's local continuity substrate — capsule access is currently namespace-gated at the top-level directory, not at the per-agent or sub-directory level, which is why the one-owner-per-instance model matters. Sub-directory namespace granularity is tracked in [#149](https://github.com/stef-k/CogniRelay/issues/149). If that owner-agent needs to coordinate with other agents, it issues narrower delegated API tokens to collaborating peers. The intended convention is that collaboration happens through the coordination surfaces (handoffs, shared artifacts, reconciliation records) rather than by treating continuity as shared common state. An agent that wants its own continuity should run its own instance.
