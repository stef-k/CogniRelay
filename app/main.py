...
def _acquire_ops_lock(repo_root: Path, job_id: str, run_id: str, started_at: str) -> Path:
    p = _ops_lock_path(repo_root, job_id)
    p.parent.mkdir(parents=True, exist_ok=True)
    try:
        fd = os.open(str(p), os.O_CREAT | os.O_EXCL | os.O_WRONLY)
    except FileExistsError as e:
        raise HTTPException(status_code=409, detail=f"Ops job already running: {job_id}") from e
    with os.fdopen(fd, "w", encoding="utf-8") as f:
        f.write(json.dumps({"job_id": job_id, "run_id": run_id, "started_at": started_at}, ensure_ascii=False))
    return p
...
def _list_ops_locks(repo_root: Path) -> list[dict[str, Any]]:
    d = safe_path(repo_root, OPS_LOCKS_DIR_REL)
    ...
    for p in sorted(d.glob("*.lock")):
        ...
        if not isinstance(row, dict):
            row = {"job_id": p.stem}
        row["path"] = f"{OPS_LOCKS_DIR_REL}/{p.name}"
        out.append(row)
    return out
...
def _ops_rotation_check(settings, lookahead_hours: int) -> dict[str, Any]:
    now = datetime.now(timezone.utc)
    lookahead = now + timedelta(hours=max(1, int(lookahead_hours)))

    token_cfg = _load_token_config(settings.repo_root)
    ...
    key_payload = _load_security_keys(settings.repo_root)
...
def _ops_replay_dead_letter_sweep(settings, auth: AuthContext, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    now = datetime.now(timezone.utc)
    dead_ids = []
    for msg_id, row in state.get("records", {}).items():
        if not isinstance(row, dict):
            continue
        if _effective_delivery_status(row, now) == "dead_letter":
            dead_ids.append(str(msg_id))
    ...
    for message_id in dead_ids:
        try:
            replay_messages(
                req=MessageReplayRequest(message_id=message_id, reason=reason, force=force),
                auth=auth,
            )
            replayed.append(message_id)
        except HTTPException as e:
            errors.append({"message_id": message_id, "status_code": e.status_code, "detail": str(e.detail)})
...
def _ops_execute_job(req: OpsRunRequest, auth: AuthContext) -> dict[str, Any]:
    settings, _ = _services()
    ...
    if req.job_id == "backup.create":
        return backup_create(req=BackupCreateRequest(**args), auth=auth)
    if req.job_id == "backup.restore_test":
        req_args = dict(args)
        if not req_args.get("backup_path"):
            latest_rel = _latest_backup_archive_rel(settings.repo_root)
            if not latest_rel:
                raise HTTPException(status_code=404, detail="No backup archive found for restore test")
            req_args["backup_path"] = latest_rel
        return backup_restore_test(req=BackupRestoreTestRequest(**req_args), auth=auth)
    if req.job_id == "replication.pull":
        return replication_pull(req=ReplicationPullRequest(**args), auth=auth)
    if req.job_id == "replication.push":
        return replication_push(req=ReplicationPushRequest(**args), auth=auth)
    if req.job_id == "messages.replay_dead_letter_sweep":
        return _ops_replay_dead_letter_sweep(settings, auth, args)
    if req.job_id == "security.rotation_check":
        return _ops_rotation_check(settings, lookahead_hours=int(args.get("lookahead_hours", 24)))
    if req.job_id == "compact.plan":
        return compact_run(req=CompactRequest(**args), auth=auth)
...
def ops_catalog(auth: AuthContext = Depends(require_auth)) -> dict:
    ...
    _audit(settings, auth, "ops_catalog", {"count": len(jobs), "client_ip": ip})
    return {
        "ok": True,
        "local_only": True,
        "security": {
...
def ops_status(limit: int = Query(default=50, ge=1, le=500), auth: AuthContext = Depends(require_auth)) -> dict:
    settings, _ = _services()
    ip = _require_local_ops_access(auth)
    runs = _load_ops_runs(settings.repo_root, limit=limit)
    locks = _list_ops_locks(settings.repo_root)
    ...
    _audit(settings, auth, "ops_status", {"limit": limit, "client_ip": ip, "runs": len(runs), "locks": len(locks)})
    return {
        "ok": True,
        "local_only": True,
        "client_ip": ip,
...
def ops_schedule_export(format: str = Query(default="systemd"), auth: AuthContext = Depends(require_auth)) -> dict:
    ...
    _audit(settings, auth, "ops_schedule_export", {"format": fmt, "client_ip": ip})
    return {
        "ok": True,
        "local_only": True,
        "format": fmt,
...
def ops_run(req: OpsRunRequest, auth: AuthContext = Depends(require_auth)) -> dict:
    settings, _ = _services()
    _enforce_rate_limit(settings, auth, "ops_run")
    _enforce_payload_limit(settings, req.model_dump(), "ops_run")
    ip = _require_local_ops_access(auth)
    ...
    try:
        job_result = _ops_execute_job(req, auth)
    except HTTPException as e:
        status = "failed"
        ...
        _audit(settings, auth, "ops_run", {"run_id": run_id, "job_id": req.job_id, "status": status, "client_ip": ip})
...
def peers_list(auth: AuthContext = Depends(require_auth)) -> dict:
    ...
    peers = []
    for peer_id, record in sorted(registry.get("peers", {}).items(), key=lambda x: x[0]):
        if not isinstance(record, dict):
            continue
        peers.append({"peer_id": peer_id, **record})
    _audit(settings, auth, "peers_list", {"count": len(peers)})
...
def peers_register(req: PeerRegisterRequest, auth: AuthContext = Depends(require_auth)) -> dict:
    settings, gm = _services()
    _enforce_rate_limit(settings, auth, "peers_register")
    _enforce_payload_limit(settings, req.model_dump(), "peers_register")
    auth.require("admin:peers")
    ...
    policies = _load_trust_policies(settings.repo_root)
    ...
    if bool(policies.get("require_fingerprint_for_trusted", True)) and target_trust == "trusted":
        fp_for_trusted = provided_fp or (prev_fp if prev_fp else None)
        if not fp_for_trusted:
    ...
    path = _write_peers_registry(settings.repo_root, registry)
    committed = gm.commit_file(path, f"peers: register {req.peer_id}")

    # Ensure trust policies are materialized in-repo for auditable governance.
    policy_path = _write_trust_policies(settings.repo_root, policies)
...
def peers_trust_transition(peer_id: str, req: PeerTrustTransitionRequest, auth: AuthContext = Depends(require_auth)) -> dict:
    settings, gm = _services()
    _enforce_rate_limit(settings, auth, "peers_trust_transition")
    _enforce_payload_limit(settings, req.model_dump(), "peers_trust_transition")
    auth.require("admin:peers")
    auth.require_write_path(PEERS_REGISTRY_REL)
    ...
    _audit(settings, auth, "peers_trust_transition", {"peer_id": peer_id, "from": current, "to": target, "reason": req.reason})
...
def peer_manifest(peer_id: str, auth: AuthContext = Depends(require_auth)) -> dict:
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"Failed to fetch peer manifest: {e}") from e

    _audit(settings, auth, "peer_manifest_fetch", {"peer_id": peer_id, "source_url": source_url})
...
def _load_trust_policies(repo_root: Path) -> dict[str, Any]:
    p = safe_path(repo_root, TRUST_POLICIES_REL)
    default = {
        "schema_version": "1.0",
        "allowed_transitions": {
...
def _write_trust_policies(repo_root: Path, payload: dict[str, Any]) -> Path:
    p = safe_path(repo_root, TRUST_POLICIES_REL)
    write_text_file(p, json.dumps(payload, ensure_ascii=False, indent=2))
    return p
